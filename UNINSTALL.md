...
## TL;DR — the scripted way (v3)

```bash
./uninstall.sh          # removes the 4 skills + auditor agent + both hooks + the engine,
                        # surgically removes both settings.json entries (backup first),
                        # restores any benched skills, KEEPS your data
...
## TL;DR — the manual way

```bash
# 1. Remove the installed skills + agent
rm -rf ~/.claude/skills/orchestra-router ~/.claude/skills/orchestra-intake \
       ~/.claude/skills/hallucination-guard ~/.claude/skills/skill-selector
rm -f ~/.claude/agents/auditor.md

# 2. Remove the hook scripts + engine (promote benched skills back first if any:
#    ls ~/.claude/orchestra/bench/)
