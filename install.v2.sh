#!/usr/bin/env bash
#
# Claude Orchestra — installer v2 (DRAFT — replaces v1 at the v2 launch commit)
#
# What v2 adds over v1:
#   • Installs the full v2 quintet: orchestra-router (chain protocol), orchestra-intake,
#     hallucination-guard, skill-selector skills + auditor agent (㉑ AUDIT conductor).
#   • Curl-pipe support: `curl -fsSL .../install.v2.sh | bash` works without git clone.
...
# refreshes the v2 managed artifacts; your own orchestra-system.md and CLAUDE.md are preserved.
#
...
# Files install.v2.sh ships, grouped by mode.
...
Usage:
  ./install.v2.sh [flags]
  curl -fsSL <url>/install.v2.sh | bash               # zero-clone install
  curl -fsSL <url>/install.v2.sh | bash -s -- --minimal
...
detect_repo_dir() {
  ...
  if [[ -z "$src" || "$src" == "bash" || "$src" == "/dev/stdin" ]]; then
    PIPE_MODE=1
    command -v git >/dev/null 2>&1 || die "curl-pipe mode requires 'git'. Install git, or clone the repo manually and run ./install.v2.sh."
...
preflight() {
  ...
  # Sanity-check the source layout — fail fast if v2 artifacts are missing from the repo.
  local missing=0
  ...
  if [[ $missing -eq 1 ]]; then
    die "Source files missing from REPO_DIR=$REPO_DIR. Are you running install.v2.sh from the repo root, or piped from the right branch?"
  fi
...
install_constitution() {
  ...
  if [[ -f "$dst" ]]; then
    ...
    skip "  v2 template available at: $src"
    return
  fi
  if [[ $DRY_RUN -eq 1 ]]; then
  ...
  cp "$src" "$dst"
  ok "constitution → rules/orchestra-system.md (v2 template — fill rosters with your tools)"
}
...
# Bash can't reliably classify 500 skills into 21 orchestras — that's an LLM job.
...
print_first_run() {
  ...
  3. Try a routing test prompt to confirm it works:
     ...
     You should see a 🎼 announcement and the nexus-strategy skill fire.

  ────────────────────────────────────────────────────────────────────────

  If anything looks wrong, every file touched has a backup with suffix
  '.bak.$STAMP' next to it. Uninstall instructions: SECURITY.md.
