...
- **2026-06-12 — v2 polish drop (while awaiting Moksh's `local-v2-work` branch).** Shipped: `uninstall.sh` (restores benched skills, surgical settings.json removal, `--purge` flag), doctor now prints the exact bench commands when over budget (never-used non-core skills, biggest savings first), v2 brand diagram (engine + telemetry feedback loop), `examples/real-prompts.md` generated from the live engine, README plain-language pass (before/after block, visible 21-orchestra roster, qmd row, new FAQs). Pending: merge of Moksh's local 37-file snapshot (committed locally by dispatch; push blocked by its sandbox — needs one terminal command), then README/registry reconciliation and a v2.0 tag.
